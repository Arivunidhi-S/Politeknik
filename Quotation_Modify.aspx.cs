...
public partial class Quotation_Modify : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                cboCalibrationQut.SelectedItem.Text = "Inhouse";
    ...
    protected void rdoButtonfeedback_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        RadioButtonList rdoButton = (RadioButtonList)sender;
        string strValue = rdoButton.SelectedItem.Text.ToString();
    ...
    public void txtDiscount_OnTextChanged(object sender, EventArgs e)
    {
        ...
        {
            double dbldiscnt = 0;
            // txtPrice.Text =((Double.Parse(txtPrice.Text)- (Double.Parse(txtPrice.Text) * Double.Parse(txtDiscount.Text) / 100)).ToString("F"));
            //  txtPrice.Text = Double.Parse(txtPrice.Text).ToString("F");
            //  decimal.Round(3, 2, MidpointRounding.AwayFromZero);
            SqlConnection conn = BusinessTier.getConnection();
            conn.Open();
    ...
    protected void cboQuotationNo_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        try
        {
            ...
            string sql1 = " select Quotation_Id,Quotation_No,Quotation_Date from Quotation where DELETED=0  and (Quot_Status='PENDING' or Quot_Status='DISCOUNT') and  Customer_ID='" + cboCustomerId.SelectedValue + "' and branch_Id='" + Session["sesBranchID"].ToString().Trim() + "' and  [Quotation_No] LIKE @text + '%' order by Quotation_No desc";
    ...
    protected void cboQuotationNo_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            SqlConnection conn = BusinessTier.getConnection();
            conn.Open();
            string sql = "select CONTACT_ID,CONTACT_PERSON,Quotation_Date,Validity,Job_Duration,Terms,Remarks,Quot_Status,Enquiry_No,Quot_Discount,feedback FROM Vw_Quotation_Customer WHERE Deleted = 0 and Quotation_ID = '" + cboQuotationNo.SelectedValue + "'";
            SqlCommand command = new SqlCommand(sql, conn);
            SqlDataReader reader = command.ExecuteReader();
            if (reader.Read())
            {
                cboContactName.Text = reader["CONTACT_PERSON"].ToString();
                cboContactName.SelectedValue = reader["CONTACT_ID"].ToString();
                //cboContractNo.Text = reader["Contract_no"].ToString();
                // cboContractNo.SelectedValue = reader["Contract_ID"].ToString();
                // cboCalibrationQut.SelectedItem.Text = reader["Calib_type"].ToString();
                DtQaotdt.SelectedDate = Convert.ToDateTime(reader["Quotation_Date"].ToString());
                txtValidity.Text = reader["Validity"].ToString();
                txtJobDuration.Text = reader["Job_Duration"].ToString();
                txtTerms.Text = reader["Terms"].ToString();
                txtRemarks.Text = reader["Remarks"].ToString();
                cbostatus.Text = reader["Quot_Status"].ToString();
                // cboEnquiryId.SelectedValue = reader["Enquiry_No"].ToString();
                // txtDiscountApprov.Text = reader["Quot_Discount"].ToString();
    ...
    protected void cboEnquiryId_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        try
        {
            SqlConnection conn = BusinessTier.getConnection();
            conn.Open();
            string sql1 = " select Enquiry_Id,Enquiry_No,Enquiry_Date from Vw_Enquiry_Customer where Customer_ID='" + cboCustomerId.SelectedValue + "' and branch_Id='" + Session["sesBranchID"].ToString().Trim() + "' and DELETED=0 and status='PENDING' and [Enquiry_No] LIKE @text + '%' order by Enquiry_No";
            SqlDataAdapter adapter1 = new SqlDataAdapter(sql1, conn);
            adapter1.SelectCommand.Parameters.AddWithValue("@Text", e.Text);
            DataTable dataTable1 = new DataTable();
            adapter1.Fill(dataTable1);
    ...
    protected void generateQuotationNo(string strWorkFrom)
    {
        ...
        string sql11 = " select Branch_ID,Branch_Code from VW_Staff_Branch_Department where deleted=0 and Staff_ID='" + Session["sesUserID"].ToString() + "'";
        ...
        if (reader11.Read())
        {
            ...
            strbrnchcode = reader11["Branch_Code"].ToString().Trim();
        }
        reader11.Close();
    ...
    protected void RadGridQuot_ItemDataBound(object sender, GridItemEventArgs e)
    {
        if (e.Item is GridDataItem)
        {
            ...
            //  string lsTip = Item["txtremarksdiscnt"].Text;
            ...
            if (string.IsNullOrEmpty(txtremarksdiscnt.Text))
            {
                txtremarksdiscnt.Text = " ";
            }
            gridItem.ToolTip = "Approved Discount :" + txtAprDiscount.Text + " %  &  Approved Remarks :" + txtremarksdiscnt.Text;

            if (Convert.ToDecimal(gridItem["Discount"].Text.ToString().Trim()) != Convert.ToDecimal(txtAprDiscount.Text))
            {
                gridItem.ForeColor = Color.Red;
        ...
        if (e.Item is GridEditableItem && e.Item.IsInEditMode)
        {
            ...
            if (!(string.IsNullOrEmpty(lbl.Text.ToString())))
            {
                ...
                SqlConnection conn = BusinessTier.getConnection();
                conn.Open();
                string sql = "select Equipment_ID,Equipment_Name,Equipment_no,calib_type FROM Vw_QuotationDetail_Equipment WHERE Deleted = 0 and Quotation_Trans_ID = '" + lbl.Text.ToString() + "'";
                SqlCommand command = new SqlCommand(sql, conn);
                SqlDataReader reader = command.ExecuteReader();
                if (reader.Read())
                {
    ...
    protected void RadGridQuot_DeleteCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
    {
        try
        {
            ...
            if (flg >= 1)
            {
                ...
                int apvsts = 0;
                string sql2 = "select count(Approve_Flag) as Apprvstatus from Quotation_Details where Quotation_ID='" + lblQuatmasterid.Text.ToString().Trim() + "' and deleted=0 and Approve_Flag=1";
                SqlCommand command1 = new SqlCommand(sql2, conn);
                SqlDataReader reader1 = command1.ExecuteReader();
                if (reader1.Read())
                {
                    apvsts = Convert.ToInt32(reader1["Apprvstatus"].ToString());
                }
                ...
                if (apvsts == 0)
                {
                    ...
                    int Quotdiscount = BusinessTier.Quotation_Discount(conn, Convert.ToInt32(lblQuatmasterid.Text), "PENDING", Convert.ToDecimal(txtDiscountApprov.Value), "", Session["sesUserID"].ToString().Trim(), "C");
                    //  BusinessTier.DisposeConnection(conn);
                }
                else
                {
                    int Quotdiscount = BusinessTier.Quotation_Discount(conn, Convert.ToInt32(lblQuatmasterid.Text), "DISCOUNT", Convert.ToDecimal(txtDiscountApprov.Value), "", Session["sesUserID"].ToString().Trim(), "C");
                }
    ...
    protected void RadGridQuot_InsertCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
    {
        ...
        try
        {
            ...
            RadNumericTextBox txtmanhours = (RadNumericTextBox)editedItem.FindControl("txtmanhours");
            RadNumericTextBox txtnoofstf_manhrs = (RadNumericTextBox)editedItem.FindControl("txtnoofstf_manhrs");
            RadNumericTextBox txtunotcost_manrs = (RadNumericTextBox)editedItem.FindControl("txtunotcost_manrs");
            RadNumericTextBox txtmileage = (RadNumericTextBox)editedItem.FindControl("txtmileage");
            RadNumericTextBox txtmileagerate = (RadNumericTextBox)editedItem.FindControl("txtmileagerate");
            RadNumericTextBox txttol = (RadNumericTextBox)editedItem.FindControl("txttol");
            RadNumericTextBox txtpetrol = (RadNumericTextBox)editedItem.FindControl("txtpetrol");
            RadNumericTextBox txtnoofrooms = (RadNumericTextBox)editedItem.FindControl("txtnoofrooms");
            RadNumericTextBox txtnoofdays_Acmds = (RadNumericTextBox)editedItem.FindControl("txtnoofdays_Acmds");
            RadNumericTextBox txtunitcost_acmds = (RadNumericTextBox)editedItem.FindControl("txtunitcost_acmds");
            RadNumericTextBox txtnoofstaff_meals = (RadNumericTextBox)editedItem.FindControl("txtnoofstaff_meals");
            RadNumericTextBox txtnoofdays_meals = (RadNumericTextBox)editedItem.FindControl("txtnoofdays_meals");
            RadNumericTextBox txtunotcost_meals = (RadNumericTextBox)editedItem.FindControl("txtunotcost_meals");

            RadNumericTextBox txtTotalmeals = (RadNumericTextBox)editedItem.FindControl("txtTotalmeals");
            RadNumericTextBox txttotalAccmdns = (RadNumericTextBox)editedItem.FindControl("txttotalAccmdns");
            RadNumericTextBox txtTotaltransport = (RadNumericTextBox)editedItem.FindControl("txtTotaltransport");
            RadNumericTextBox txttotmanhours = (RadNumericTextBox)editedItem.FindControl("txttotmanhours");
            RadNumericTextBox txtmileagecost = (RadNumericTextBox)editedItem.FindControl("txtmileagecost");
            ...
            {
                cboCalibration.Text = cboCalibrationQut.Text;
            ...
            int flg = BusinessTier.SaveInsertQuot_Detail(conn, 1, Convert.ToInt32(lblQuatmasterid.Text), txtremarksdetails.Text, Convert.ToInt32(cboEquipmentId.SelectedValue), Convert.ToInt32(txtQty.Text), Convert.ToDecimal(txtPrice.Text), Convert.ToInt32(txtadditional.Text), Convert.ToDecimal(txtDiscount.Text), cboCalibration.Text.ToString().Trim(), txtDesc.Text.ToString().Trim(), aprvfalg, 1, Session["sesUserID"].ToString(), "N");
    ...
    protected void RadGrid1_ItemCommand(object source, GridCommandEventArgs e)
    {
        try
        {
            ...
            if (e.CommandName == "RowClick" && e.Item is GridDataItem)
            {
                ...
                if (rdoButton.SelectedItem.Text.ToString().Trim() == "Contract")
                {
                    flg = BusinessTier.SaveInsertQuot_Detail(connsave, 1, Convert.ToInt32(lblQuatmasterid.Text), "1 to 2 Weeks", Convert.ToInt32(strDetailId), 3, Convert.ToDecimal(intprice), 0, 0, cboCalibrationQut.Text, "", 0, 1, Session["sesUserID"].ToString().Trim(), "C");
                }
                else
                {
                    flg = BusinessTier.SaveInsertQuot_Detail(connsave, 1, Convert.ToInt32(lblQuatmasterid.Text), "1 to 2 Weeks", Convert.ToInt32(strDetailId), 3, 200, 0, 0, cboCalibrationQut.Text, "", 0, 1, Session["sesUserID"].ToString().Trim(), "SaveInsertQuot_DetailE");
    ...
    public DataTable DataSourceHelper(string typeMasterDetail, string strMasterId)
    {
        ...
        {
            sql = "select * FROM Vw_QuotationDetail_Equipment WHERE Deleted=0 and UserID='" + Session["sesUserID"].ToString() + "'  order by Created_Date desc";
        }

        else if (typeMasterDetail == "T")
        {
            sql = "select * FROM Vw_QuotationDetail_Equipment WHERE Deleted=0 and CREATED_BY='" + Session["sesUserID"].ToString() + "' and  Quotation_Id='" + lblQuatmasterid.Text + "'  order by Created_Date desc";

        }
        else if (typeMasterDetail == "F")
        {
            sql = "select * FROM Vw_QuotationDetail_Equipment WHERE Deleted=0 and CREATED_BY='" + Session["sesUserID"].ToString() + "'";
    ...
    protected void btnDel_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            sql = "select max (Quotation_ID) as Q_ID FROM Quotation WHERE Deleted=0";
            SqlCommand cmd2 = new SqlCommand(sql, conn);
            SqlDataReader rd2 = cmd2.ExecuteReader();
            if (rd2.Read())
            {
                Q_ID = rd2["Q_ID"].ToString().Trim();
            }
            rd2.Close();

            if (Q_ID == cboQuotationNo.SelectedValue.ToString().Trim())
            {
                BusinessTier.Quotation_Delete(conn, Convert.ToInt32(cboQuotationNo.SelectedValue.ToString()), Session["sesUserID"].ToString());
                lblStatus.Text = "Last Quotation Sucessfully Deleted !";
                lblStatus.ForeColor = Color.Yellow;
            }
            else
            {
                lblStatus.Text = "Last Quotation only Delete";
            }
        ...
        catch (Exception ex)
        {
            ShowMessage(9);
            //InsertLogAuditTrial is used to insert the action into MasterAuditTrail table
            InsertLogAuditTrail(Session["sesUserID"].ToString(), "Quotation_Modify", "btnDel_Click", ex.ToString(), "Audit");
        }
        //try
        //{
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        ...
        {
            int ApprvFlag = 0;
            if (string.IsNullOrEmpty(txtDiscountApprov.Text.ToString().Trim()))
            {
                txtDiscountApprov.Text = "0";
            }
            if (string.IsNullOrEmpty(cbostatus.Text.ToString().Trim()) || (cbostatus.Text.ToString().Trim() == "--Select Status--"))
            {
                lblStatus.Text = "Please Select the Status";
                return;

            }

            else if (string.IsNullOrEmpty(cboQuotationNo.Text.ToString().Trim()) || (string.IsNullOrEmpty(cboCustomerId.Text.ToString().Trim())))
            {
                lblStatus.Text = "Please Select the CustomerName and Quotation No";
            }


            else
            {
                if (cbostatus.Text.ToString().Trim() == "UNSUCCESSFUL")
                {

                    int intgetQuotMasterId = BusinessTier.SaveQuotation_Master(conn, Convert.ToInt32(cboQuotationNo.SelectedValue), Convert.ToInt32(Session["sesBranchID"].ToString().Trim()), cboQuotationNo.Text, Convert.ToInt32(cboCustomerId.SelectedValue), Convert.ToInt32(cboContactName.SelectedValue), Convert.ToInt32(cboContractNo.SelectedValue), DtQaotdt.SelectedDate.Value, txtTerms.Text.ToString().Trim(), Convert.ToInt32(txtValidity.Text.ToString().Trim()), txtJobDuration.Text.ToString().Trim(), cbostatus.Text, "1", "", txtRemarks.Text.ToString().Trim(), Convert.ToDecimal(txtDiscountApprov.Text.ToString().Trim()), txtfeedback.Text.ToString().Trim(), Session["sesUserID"].ToString().Trim(), "U");
                    ShowMessage(75);
                }
                else if (cbostatus.Text.ToString().Trim() == "PENDING")
                {
                    int apvsts = 0;
                    string sql2 = "select count(Approve_Flag) as Apprvstatus from Quotation_Details where Quotation_ID='" + lblQuatmasterid.Text.ToString().Trim() + "' and deleted=0 and Approve_Flag=1";
                    SqlCommand command1 = new SqlCommand(sql2, conn);
                    ...
                    if (apvsts == 0)
                    {
                        int intgetQuotMasterId = BusinessTier.SaveQuotation_Master(conn, Convert.ToInt32(cboQuotationNo.SelectedValue), 1, cboQuotationNo.Text, Convert.ToInt32(cboCustomerId.SelectedValue), Convert.ToInt32(cboContactName.SelectedValue), 1, DtQaotdt.SelectedDate.Value, txtTerms.Text.ToString().Trim(), Convert.ToInt32(txtValidity.Text.ToString().Trim()), txtJobDuration.Text.ToString().Trim(), cbostatus.Text, "1", "", txtRemarks.Text.ToString().Trim(), Convert.ToDecimal(txtDiscountApprov.Text.ToString().Trim()), txtfeedback.Text.ToString().Trim(), Session["sesUserID"].ToString().Trim(), "U");
                    }
                    else
                    {
                        int intgetQuotMasterId = BusinessTier.SaveQuotation_Master(conn, Convert.ToInt32(cboQuotationNo.SelectedValue), 1, cboQuotationNo.Text, Convert.ToInt32(cboCustomerId.SelectedValue), Convert.ToInt32(cboContactName.SelectedValue), 1, DtQaotdt.SelectedDate.Value, txtTerms.Text.ToString().Trim(), Convert.ToInt32(txtValidity.Text.ToString().Trim()), txtJobDuration.Text.ToString().Trim(), "DISCOUNT", "1", "", txtRemarks.Text.ToString().Trim(), Convert.ToDecimal(txtDiscountApprov.Text.ToString().Trim()), txtfeedback.Text.ToString().Trim(), Session["sesUserID"].ToString().Trim(), "U");
                    }
                    ShowMessage(75);
                ...
                else
                {
                    ...
                    if (k == 1)
                    {
                        //  int intgetQuotMasterId = BusinessTier.SaveQuotation_Master(conn, Convert.ToInt32(cboQuotationNo.SelectedValue), Convert.ToInt32(Session["sesBranchID"].ToString().Trim()), cboQuotationNo.Text, Convert.ToInt32(cboCustomerId.SelectedValue), Convert.ToInt32(cboContactName.SelectedValue), Convert.ToInt32(cboContractNo.SelectedValue), DtQaotdt.SelectedDate.Value, txtTerms.Text.ToString().Trim(), Convert.ToInt32(txtValidity.Text.ToString().Trim()), txtJobDuration.Text.ToString().Trim(), "DISCOUNT", cboEnquiryId.SelectedValue, "", txtRemarks.Text.ToString().Trim(), Convert.ToInt32(txtDiscountApprov.Text.ToString().Trim()), "", Session["sesUserID"].ToString().Trim(), "U");
                    ...
                    foreach (GridDataItem item in RadGridQuot.Items)
                    {
                        ...
                        if (reader2.Read())
                        {
                            string stradtnlrng = reader2["ADDITIONAL_RANGE"].ToString();
                        ...
                        int flg = BusinessTier.SaveInsertQuot_Detail(conn, Convert.ToInt32(ID.ToString()), Convert.ToInt32(lblQuatmasterid.Text), "", 1, 1, Convert.ToDecimal(dcmlprice), 1, Convert.ToDecimal(txtDiscountApprov.Text), "", "", ApprvFlag, 1, Session["sesUserID"].ToString(), "A");
