...
public partial class DeliveryEquipment_Modify : System.Web.UI.Page
{
    ...
    protected void cboJobno_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        try
        {
            SqlConnection conn = BusinessTier.getConnection();
            conn.Open();
            string sql1 = "select distinct(jobno) as refno FROM Vw_CRMReceiveEquipmentCustTransDetails WHERE Deleted=0 and orderno=0 and Delivery=1 and Quotation_Id = '" + cboQuotationNo.SelectedValue.ToString() + "' group by RECEIVED_TRANS_DETAIL_ID,jobno,runningno ";
            SqlDataAdapter adapter1 = new SqlDataAdapter(sql1, conn);
            DataTable dataTable1 = new DataTable();
            adapter1.Fill(dataTable1);
            RadComboBox comboBox = (RadComboBox)sender;
    ...
    protected void cboRefNo_OnItemsRequested(object sender, RadComboBoxItemsRequestedEventArgs e)
    {
        try
        {
            SqlConnection conn = BusinessTier.getConnection();
            conn.Open();
            string sql1 = "select distinct(DeliveryNo) as dno FROM Vw_CRMReceiveEquipmentCustTransDetails WHERE Deleted=0 and orderno=0 and Delivery=1 and Quotation_Id = '" + cboQuotationNo.SelectedValue.ToString() + "' and jobno='" + cbojobno.Text.ToString() + "' group by RECEIVED_TRANS_DETAIL_ID,DeliveryNo,runningno ";
            SqlDataAdapter adapter1 = new SqlDataAdapter(sql1, conn);
            DataTable dataTable1 = new DataTable();
            adapter1.Fill(dataTable1);
            RadComboBox comboBox = (RadComboBox)sender;
    ...
    protected void cboRefNo_SelectedIndexChanged(object sender, EventArgs e)
    {
        SqlConnection conn = BusinessTier.getConnection();
        conn.Open();
        string refno = cbojobno.Text.ToString();
        string deliveryno = cboRefNo.Text.ToString();
        try
        {
            RadGridQuot.DataSource = DataSourceHelper(refno, deliveryno);
            RadGridQuot.Rebind();
    ...
    protected void RadGridQuot_NeedDataSource(object source, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        try
        {
            RadGridQuot.DataSource = DataSourceHelper("0", "0");
        }
        catch (Exception ex)
        {
            ShowMessage(9);
    ...
    public DataTable DataSourceHelper(string refno, string deliveryno)
    {
        SqlConnection conn = BusinessTier.getConnection();
        conn.Open();
        string sql = "";

        sql = "select * FROM Vw_CRMReceiveEquipmentCustTransDetails WHERE Deleted=0 and jobno='" + refno.ToString() + "' and DeliveryNo='" + deliveryno.ToString() + "' and Delivery=1 and Invoiceflag=0";
    ...
    protected void btnReport_Click(object sender, EventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(Page, typeof(Page), "OpenWindow", "window.open('Report_AfterCalibration.aspx?param1=" + jbno.ToString().Trim() + "&param2=" + cboRefNo.Text.ToString().Trim() + "');", true);
    }
    ...
    protected void btnRegister_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            foreach (GridDataItem grdItem in RadGridQuot.Items)
            {
                ...
                {
                    BusinessTier.Delivery_Save(conn, intTrackId, 0, cboRefNo.Text.ToString());
                    lblStatus.Text = "Successfully Delivery of Equipment Changed";
                    lblStatus.ForeColor = Color.Yellow;
                }
                else
                {
                    BusinessTier.Delivery_Save(conn, intTrackId, 1, cboRefNo.Text.ToString());
                }
            }
